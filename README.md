...
등장 장소도 별도 페이지로 분리됩니다:
- `locations.html` : 등장 장소 목록 (배경 이미지 카드, type별 분류)
- `location.html?id=<장소id>` : 개별 장소 상세 (상단 배경 이미지 + 설명 + 관련 세션)
...
├── locations.html          # 등장 장소 목록
├── location.html           # 개별 등장 장소 상세 (?id=)
├── logs/                   # 세션 로그 HTML 파일들
│   ├── session01.html
│   ├── session02.html
│   ├── session03.html
...
│   ├── oneshot01.html
│   └── frost01.html
├── characters/             # 등장인물 portrait/standing 이미지
├── locations/              # 등장 장소 배경 이미지
├── assets/
...
│       ├── locations.js    # 장소 목록 렌더링
│       ├── location-detail.js   # 장소 상세 렌더링
...
1. `locations/` 폴더에 배경 이미지 파일 추가 (예: `village.png`)
2. `assets/js/data.js`의 `LOCATIONS` 배열에 항목 추가:
   ```js
   {
     id: "village",
     name: "마을 이름",
     type: "도시",
     image: "locations/village.png",
     summary: "목록 카드용 짧은 설명",
     description: "상세 페이지용 긴 설명 (선택)",
     relatedSessions: [["main", 1]]  // [campaignId, num] 튜플 (선택)
   }
   ```
...
- 등장 장소 상세: `location.html?id=red-moon-inn`
